...
/**
 * Filter out the players ( and sort them based on their number )
 */
$competition_players = array_filter($attributes['teamMembers'], function ($team_member) {
	return (in_array($team_member['role'], array('player', 'captain', 'libero')));
});

/**
 * Filter out the training players
 */
$training_players = array_filter($attributes['teamMembers'], function ($team_member) {
	return ($team_member['role'] === 'trainee');
});

/**
 * Filter out the supervisors
 */
$supervisors = array_filter($attributes['teamMembers'], function ($team_member) {
	return (in_array($team_member['role'], array('coach-trainer', 'coach', 'assistant-coach', 'trainer', 'assistent-trainer')));
});

/**
 * The array of team member roles (slugs and names)
...
<section <?= get_block_wrapper_attributes(); ?>>
	...
	<div class="grid sm:grid-cols-2 gap-6 pb-8">
		<?php foreach ($competition_players as $player) {
			ob_start(); ?>
			<dd class="flex gap-2 justify-start items-center">
				...
				<div class="relative">
					<?php switch ($player['role']) {
						case 'player':
							ob_start(); ?>
							<svg viewBox="0 0 32 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-10 inline-block">
				...
				<div class="flex flex-col">
					...
					<span class="text-sm text-neutral-500"><?= array_key_exists($player['role'], $teamMemberRoles) ? $teamMemberRoles[$player['role']] : '–' ?></span>
				</div>
			</dd>
		<?= ob_get_clean();
		} ?>
	...
	<div class="grid sm:grid-cols-2 gap-6 pb-8">
		<?php if (!empty($training_players)) {
			foreach ($training_players as $player) {
				ob_start(); ?>
				<dd class="flex gap-2 justify-start items-center">
					<!-- User icon -->
					<svg viewBox="0 0 32 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-10 inline-block">
	...
	<div class="grid sm:grid-cols-2 gap-6 pb-8">
		<?php foreach ($supervisors as $supervisor) {
			...
			<dd class="flex gap-2 justify-start items-center">
				<!-- User icon -->
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#3D3D3D" class="w-10 inline-block">
					<path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd" />
				</svg>
				<!-- Naam en role -->
				<div class="flex flex-col">
					<span class="text-base text-neutral-900"><?= $supervisor['name'] ?></span>
					<span class="text-sm text-neutral-500"><?= array_key_exists($player['role'], $teamMemberRoles) ? $teamMemberRoles[$supervisor['role']] : '–' ?></span>
				</div>
			</dd>
		<?= ob_get_clean();
		} ?>
